...
#
# RX descriptor stride agreement gate (Phase A1 / RSS_RXHASH plan).
#
...
for fn in desc_read_rx desc_write_rx desc_publish_own; do
	body=$(awk "/pub\(crate\) fn $fn\(/,/^}/" "$UB")
	if [[ -z "$body" ]]; then
		red "$fn: not found in unsafe_boundary.rs"
		continue
	fi
	# 1. must take a format: RxDescFormat parameter.
	...
	# 2. must derive stride from format.descriptor_len().
	...
	# 3. must NOT take a typed *mut RxDescriptor ring pointer (32B-unconditional
	#    indexing is the exact bug this gate prevents).
