...
[`BRINGUP_RUNBOOK.md`](BRINGUP_RUNBOOK.md) which targets Controller).
...
- **ASPM-on 24h L1.x idle soak**: **Gateway only**. The KVM guest's synthetic upstream PCIe bridge advertises ASPM L0s exclusively, so even with `force_aspm=1` setting Config5 ASPM_en=1 the link cannot enter L1.x. This is the historical lockup gate; it must clear on bare metal to be considered honestly cleared. See `docs/HARDENING_CLOSEOUT.md` and `docs/GATEWAY_SETUP.md`.
- Suspend/resume cycles: gated by the kernel-Rust PCI PM API gap (`docs/PM_GAP.md`). Once the API exists, full cycles run on Gateway against real ACPI S3.
...
- Multi-queue + RSS sub-feature is **N/A on 8125B** at the hardware level (`docs/MULTIQUEUE_RSS.md`) regardless of environment; the per-feature gate is vacuously satisfied. M6 actually delivers MSI-X + jumbo + the existing CSUM/TSO (already done in M4-perf) on this chip.
